    ...
    def update(
        ...
        *,
        ...
        for uid in agents_to_train:
            ...
            batch_actions = torch.stack(buffer.actions).to(cfg.SIM.DEVICE).reshape(-1)
            batch_log_probs = torch.stack(buffer.log_probs).to(cfg.SIM.DEVICE).reshape(-1)
            batch_advantages = advantages.to(cfg.SIM.DEVICE).reshape(-1)
            batch_returns = returns.to(cfg.SIM.DEVICE).reshape(-1)
            batch_values = torch.stack(buffer.values).to(cfg.SIM.DEVICE).reshape(-1)
