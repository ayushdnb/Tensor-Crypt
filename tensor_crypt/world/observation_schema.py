...
def distance_to_center_norm(positions: torch.Tensor, grid_w: int, grid_h: int) -> torch.Tensor:
    ...
    max_distance = math.sqrt(center[0].item() ** 2 + center[1].item() ** 2)
    return torch.clamp(distances / max(max_distance, 1e-6), 0.0, 1.0)
...
def build_observation_bundle(
    *,
    ...
    global_alive = registry.get_alive_indices()
    if len(global_alive) == 0:
        alive_fraction = torch.zeros(len(alive_indices), device=device)
        mean_mass_norm = torch.zeros(len(alive_indices), device=device)
        mean_hp_ratio = torch.zeros(len(alive_indices), device=device)
    else:
        ...
        mean_mass_norm = torch.full(
            (len(alive_indices),),
            global_mass_norm.mean().item(),
            device=device,
        )
        mean_hp_ratio = torch.full(
            (len(alive_indices),),
            global_hp_ratio.mean().item(),
            device=device,
        )
