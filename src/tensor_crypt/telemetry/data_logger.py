    ...
    def log_tick_summary(
        ...
        *,
        ...
        if num_alive > 0:
            ...
            for slot_idx, uid, hp_ratio in zip(alive_slots, slot_uids, summary_surface_cpu[:, 0].tolist()):
                uid_int = int(uid)
                ...
                max_lineage_depth_alive = max(
                    max_lineage_depth_alive,
                    int(registry.uid_generation_depth.get(uid_int, 0)),
                )
        ...
        if emit_tick_summary:
            ...
            if summary_surface_cpu is not None:
                stats.update(
                    {
                        "mean_hp_ratio": float(summary_surface_cpu[:, 0].mean().item()),
                        "mean_mass": float(summary_surface_cpu[:, 1].mean().item()),
                        "mean_vision": float(summary_surface_cpu[:, 2].mean().item()),
                        "mean_metabolism": float(summary_surface_cpu[:, 3].mean().item()),
                    }
                )
                for family_id, count in family_counts.items():
