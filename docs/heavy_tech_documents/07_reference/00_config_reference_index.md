...
> Scope: Provide a lookup-first index of configuration sections, their safety classes, and selected field meanings derived from the uploaded runtime config dataclasses.
...
| `TICKS_PER_SEC` | `int` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.SIM.TICKS_PER_SEC` runtime read was found in the code dump. Vie... |
...
| `EXPOSE_H_GRAD` | `bool` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.GRID.EXPOSE_H_GRAD` runtime read was found in the code dump. Gr... |
...
| `MODE` | `str` | guarded compatibility surface | Prompt 5 reproduction control surface.  guarded compatibility surface. Non-supported non-default values are rejected during runtime validation rather than be... |
| `BRAIN_PARENT_SELECTOR` | `str` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.RESPAWN.BRAIN_PARENT_SELECTOR` runtime read was found in the co... |
| `TRAIT_PARENT_SELECTOR` | `str` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.RESPAWN.TRAIT_PARENT_SELECTOR` runtime read was found in the co... |
...
| `FLOOR_RECOVERY_REQUIRE_TWO_PARENTS` | `bool` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.RESPAWN.FLOOR_RECOVERY_REQUIRE_TWO_PARENTS` runtime read was fo... |
...
| `ASSERT_BINARY_PARENTING` | `bool` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.RESPAWN.ASSERT_BINARY_PARENTING` runtime read was found in the ... |
...
| Field | Type | Safety class | Note |
| --- | --- | --- | --- |
| `INIT` | `TraitInit` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.TRAITS.INIT` runtime read was found in the code dump. Legacy in... |
...
| `MOVE_FAIL_COST` | `float` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.PHYS.MOVE_FAIL_COST` runtime read was found in the code dump. C... |
...
| `RAY_FIELD_AGG` | `str` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.PERCEPT.RAY_FIELD_AGG` runtime read was found in the code dump.... |
| `RAY_STEP_SAMPLER` | `str` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.PERCEPT.RAY_STEP_SAMPLER` runtime read was found in the code du... |
...
| `LEGACY_ADAPTER_MODE` | `str` | active runtime knob | active runtime knob. Legacy adapter identity string. The configured default names the bridge implementation: `"prompt2_canonical_bridge_v1"`. Treat this prim... |
...
| `TRACK_TRAINING_STATE` | `bool` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.PPO.TRACK_TRAINING_STATE` runtime read was found in the code du... |
...
| Field | Type | Safety class | Note |
| --- | --- | --- | --- |
| `SELECTION` | `str` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.EVOL.SELECTION` runtime read was found in the code dump. Docume... |
...
| `FITNESS_TEMP` | `float` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.EVOL.FITNESS_TEMP` runtime read was found in the code dump. Doc... |
| `TRAIT_LOGIT_MUTATION_SIGMA` | `float` | active runtime knob | Prompt 5 mutation knobs.  active runtime knob. Standard deviation for ordinary latent trait-logit mutations. Higher values produce more aggressive trait drift. |
...
| `PAINT_BRUSH` | `List[int]` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.VIEW.PAINT_BRUSH` runtime read was found in the code dump. Docu... |
...
| `CELL_SIZE` | `int` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.VIEW.CELL_SIZE` runtime read was found in the code dump. Docume... |
...
| `SHOW_CATASTROPHE_PANEL` | `bool` | active runtime knob | Prompt 6 viewer catastrophe surfaces.  active runtime knob. Whether the viewer shows the catastrophe panel. Presentation-only. |
...
| `ASSERT_NO_SLOT_OWNERSHIP_LEAK` | `bool` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.IDENTITY.ASSERT_NO_SLOT_OWNERSHIP_LEAK` runtime read was found ... |
...
| `ATOMIC_WRITE_ENABLED` | `bool` | active runtime knob | Prompt 7 atomic publish and corruption controls.  active runtime knob. Whether checkpoint publish uses temp-file + atomic replace semantics. Recommended to k... |
...
| `VALIDATION_STRICTNESS` | `str` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.VALIDATION.VALIDATION_STRICTNESS` runtime read was found in the... |
...
| `SAVE_LOAD_SAVE_COMPARE_BUFFERS` | `bool` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.VALIDATION.SAVE_LOAD_SAVE_COMPARE_BUFFERS` runtime read was fou... |
| `STRICT_TELEMETRY_SCHEMA_WRITES` | `bool` | currently unread / effectively dead | currently unread / effectively dead in the uploaded repository dump. Audit basis: no direct `.VALIDATION.STRICT_TELEMETRY_SCHEMA_WRITES` runtime read was fou... |
...
| `TYPE_PARAMS` | `Dict[str, Dict[str, float]]` | active runtime knob | Prompt 6 catastrophe intensity and scheduler knobs.  active runtime knob. Per-catastrophe parameter bundle. This is the main intensity-and-shape surface for ... |
