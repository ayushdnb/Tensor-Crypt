...
2. public validation helpers, packaging metadata, and any validation assets present in the working tree being reviewed
3. official external sources, only where narrowly needed
4. general background knowledge, only for foundations content

The present corpus build is grounded in a directly inspectable repository tree. Implementation claims are therefore written against checked-in source, public validation helpers, packaging metadata, and repository metadata rather than against detached secondary material.
