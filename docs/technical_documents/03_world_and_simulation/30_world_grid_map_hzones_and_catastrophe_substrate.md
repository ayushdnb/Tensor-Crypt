...
- tests including `tests/test_grid_and_mapgen.py`, `tests/test_catastrophes.py`, and `tests/test_catastrophe_scheduler_controls.py`
...
The world always includes border walls created by the grid implementation. Additional random walls are generated by `tensor_crypt.world.procedural_map.add_random_walls`, which creates one-cell-thick random segments.
