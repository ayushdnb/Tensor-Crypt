...
def test_prompt7_atomic_checkpoint_emits_manifest_and_latest_pointer(runtime_builder, tmp_path):
    runtime = runtime_builder(seed=21, agents=6, walls=0, hzones=0)
    bundle = capture_runtime_checkpoint(runtime)
    checkpoint_path = tmp_path / "prompt7_checkpoint.pt"
...
def test_prompt7_checkpoint_checksum_corruption_is_detected(runtime_builder, tmp_path):
    ...
    with checkpoint_path.open("ab") as handle:
        handle.write(b"prompt7-corruption")

    with pytest.raises(ValueError):
        load_runtime_checkpoint(checkpoint_path)

def test_prompt7_latest_pointer_references_published_bundle(runtime_builder, tmp_path):
