...
def test_initial_population_bootstraps_birth_and_open_life_ledgers(runtime_builder):
...
def test_death_finalization_emits_taxonomy_and_life_row(runtime_builder):
...
def test_death_ledger_handles_null_then_non_null_killer_uid(runtime_builder):
...
def test_lineage_export_contains_parent_role_edges(runtime_builder):
...
def test_tick_and_family_summaries_include_extended_fields(runtime_builder):
...
def test_viewer_trait_surface_matches_trait_latent(runtime_builder):
